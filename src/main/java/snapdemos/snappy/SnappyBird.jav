...
public class SnappyBird extends ViewOwner {
    ...
    Image        PIPE_IMAGE = Image.get("http://reportmill.com/codefun/SnappyPipe.png");
...
protected View createUI()
{
    ...
    Image fimg = Image.get("http://reportmill.com/codefun/Snappy.png");
    _flappy = new ImageView(fimg); _flappy.setBounds(180, 180, 64, 64);
...
void addPipe()
{
    ...
    VBox vbox = new VBox(); vbox.setAlign(Pos.TOP_CENTER); vbox.setPrefHeight(_view.getPrefHeight());
...
View createPipePart()
{
    ImageView pipe = new ImageView(PIPE_IMAGE); pipe.setFitMinor(true); pipe.setFitAlways(true);
    pipe.setBorder(new Color("#008000"), 2);
    return pipe;
}

void resetPipe(VBox aPipe)
{
    // Reset pipe levels
...
public static void main(String args[])  { new SnappyBird().setWindowVisible(true); }
